...
const directory = 'docs/quality/a3';
...
try {
  ...
  const network = await page.evaluate(() => performance.getEntriesByType('resource').map(entry => ({ name: entry.name, transfer: entry.transferSize })));
  const result = await lighthouse('http://127.0.0.1:4321/es/', { port: 9222, output: ['json', 'html'], logLevel: 'error', onlyCategories: ['performance', 'accessibility', 'best-practices', 'seo'] });
  const report = {
    phase, date: new Date().toISOString(), assets, network,
  ...
  await writeFile(`${directory}/${phase}-lighthouse.html`, result.report[1]);
