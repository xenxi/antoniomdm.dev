...
const cvSupportingLine = model.profile.focusLine;
...
function getProfile(locale) {
  ...
  return {
    ...value,
    headline: localize(value.headline, locale),
    summary: localize(value.summary, locale),
    education: value.education.map((item) => localize(item, locale)),
    languages: value.languages.map((item) => localize(item, locale)),
  };
}

function getExperience(locale) {
  return model.experiences.map((item) => ({
    ...item,
    company: localize(item.company, locale),
    role: localize(item.role, locale),
    period: period(item.start, item.end, locale),
    summary: localize(item.summary, locale),
  }));
}

function getCompetencies(locale) {
  return model.competencies.map((item) => ({
    ...item,
...
function getAchievements(locale) {
  return model.achievements.map((item) => ({
    ...item,
...
function availableUrl(id) {
  ...
  if (!link || link.availability !== "available" || !link.url)
    throw new Error(`Contact link "${id}" is not available`);
  return link.url;
}
...
const contactSummary = (locale) =>
  `${contactInfo.location[locale]} · ${contactInfo.availability[locale]}`;
...
function pdfText(text) {
  return [...String(text)]
    .map(
      (char) => PDF_FALLBACK[char] ?? (char.codePointAt(0) > 255 ? "?" : char),
    )
    .join("")
...
function buildLines(locale) {
  ...
  const labels =
    locale === "es"
      ? {
          ...
          skills: "COMPETENCIAS",
          achievements: "LOGROS CONFIRMADOS",
          education: "FORMACIÓN",
          languages: "IDIOMAS",
        }
      : {
  ...
  const lines = [];
  const line = (runs, options = {}) =>
    lines.push({
      runs,
      leading: options.leading ?? 11,
      spaceBefore: options.spaceBefore ?? 0,
      rule: options.rule ?? false,
      wrap: options.wrap ?? USABLE,
    });
  const paragraph = (text, options = {}) => {
    const font = options.font ?? "F1";
    const size = options.size ?? 8.8;
    const leading = options.leading ?? size * 1.28;
    for (const value of wrapText(text, font, size, options.width ?? USABLE))
      line([{ text: value, font, size, link: options.link }], { leading });
  };
  const heading = (text) => {
    lines.push({ spacer: 7 });
  ...
  line([{ text: profile.headline, font: "F2", size: 11 }], {
    leading: 14,
    spaceBefore: 1,
  });
  paragraph(cvSupportingLine[locale], { font: "F3", size: 9.4, leading: 12 });
  lines.push({ spacer: 5 });
  for (const contact of [
    ...
    line([{ text: contact.text, font: "F1", size: 8.7, link: contact.link }], {
      leading: 11,
    });
  ...
  for (const job of experiences) {
    line([{ text: job.role, font: "F2", size: 9.2 }], {
      leading: 11.4,
      spaceBefore: 3,
    });
    line(
      [
        { text: job.company, font: "F1", size: 8.5 },
        { text: "  ·  ", font: "F1", size: 8.5 },
        { text: job.period, font: "F3", size: 8.5 },
      ],
      { leading: 10.6 },
    );
    paragraph(job.summary, { size: 8.5, leading: 10.5 });
    if (job.id === "domingo-alonso" || job.id === "anexia")
      for (const section of job.sections)
        paragraph(section.content[locale], { size: 8.5, leading: 10.5 });
  }
  ...
  for (const item of achievements)
    paragraph(`${item.title}: ${item.summary} ${item.scope[locale]}`, {
      size: 8.5,
      leading: 10.5,
    });

  heading(
    locale === "es" ? "DECISIONES DE ARQUITECTURA" : "ARCHITECTURE DECISIONS",
  );
  for (const id of [
    ...
    const decision = model.representativeDecisions.find(
      (item) => item.id === id,
    );
    paragraph(`${decision.title[locale]}: ${decision.decision[locale]}`, {
      size: 8.8,
      leading: 12,
    });
  }
  ...
  heading(
    locale === "es" ? "LABORATORIO PERSONAL" : "PERSONAL ENGINEERING LAB",
  );
  const lab = model.projectCatalog.find((item) => item.id === "platform934");
  paragraph(`${lab.publicName[locale]} — ${lab.content.purpose[locale]}`, {
    size: 8.5,
    leading: 10.5,
  });
  paragraph(lab.content.tradeoff[locale], { size: 8.5, leading: 10.5 });
...
function layout(lines) {
  ...
  for (const entry of lines) {
    ...
    if (entry.spacer) {
      if (y - entry.spacer < BOTTOM) flush();
      else y -= entry.spacer;
      continue;
    }
    if (entry.spaceBefore) {
      if (y - entry.spaceBefore < BOTTOM) flush();
      else y -= entry.spaceBefore;
    }
  ...
  return (
    page.lines
      .map((entry) => {
        ...
        if (entry.rule)
          operations += `0.6 w 0.4 0.4 0.4 RG ${LEFT} ${(entry.y - 4).toFixed(2)} m ${RIGHT} ${(entry.y - 4).toFixed(2)} l S\n`;
        return operations;
      })
      .join("") + `BT /F1 8 Tf 288 28 Td (${index + 1} / ${total}) Tj ET\n`
  );
}
...
function buildPdf(pages, info) {
  ...
  pages.forEach((page, index) =>
    page.links.forEach((link) =>
      annotations.push({ ...link, pageIndex: index, number: next++ }),
    ),
  );
  ...
  pages.forEach((page, index) => {
    const refs = annotations
      .filter((annotation) => annotation.pageIndex === index)
      .map((annotation) => `${annotation.number} 0 R`);
    const annots = refs.length ? ` /Annots [${refs.join(" ")}]` : "";
    set(
      pageObjectNumbers[index],
      `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}] /Resources << /Font << /F1 3 0 R /F2 4 0 R /F3 5 0 R >> >> /Contents ${contentObjectNumbers[index]} 0 R${annots} >>`,
    );
  ...
  for (const annotation of annotations) {
    ...
    set(
      annotation.number,
      `<< /Type /Annot /Subtype /Link /Rect [${x.toFixed(2)} ${y.toFixed(2)} ${(x + w).toFixed(2)} ${(y + h).toFixed(2)}] /Border [0 0 0] /A << /S /URI /URI (${pdfText(annotation.url)}) >> >>`,
    );
  }
  const now = new Date();
  ...
  set(
    infoNumber,
    `<< /Title (${pdfText(info.title)}) /Author (${pdfText(info.author)}) /Subject (${pdfText(info.subject)}) /Creator (Anto\\361iOS) /Producer (Anto\\361iOS) /CreationDate (D:${stamp}) >>`,
  );
...
for (const locale of ["es", "en"]) {
  ...
  const buffer = buildPdf(pages, {
    ...
    author: profile.name,
    subject: cvSupportingLine[locale],
  });
  const target = resolve(root, outputs[locale]);
  mkdirSync(dirname(target), { recursive: true });
  writeFileSync(target, buffer);
  console.log(
    `${outputs[locale]}: ${pages.length} pages, ${buffer.length} bytes`,
  );
}

console.log(
  "PHONE_NOT_VERIFIED_FOR_PDF: no approved telephone value was found in the repository; telephone omitted from both PDFs.",
);
