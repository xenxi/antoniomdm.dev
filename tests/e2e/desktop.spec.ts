...
test('main routes and machine-readable outputs respond', async ({ request }) => {
  for (const path of ['/', '/projects/', '/projects/platform934/', '/experience/', '/notes/', '/notes/os-foundation/', '/lab/', '/about/', '/cv/', '/arcade/', '/settings/', '/terminal/', '/rss.xml', '/sitemap.xml', '/robots.txt', '/llms.txt', '/cv.txt', '/social.png']) {
...
test('project filters, note content, lab concepts and printable CV', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Explore concept' }).first().click(); await expect(page.locator('.lab-concept')).toContainText('Arrivals');
