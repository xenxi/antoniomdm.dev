...
test('keyboard movement, collision, touch controls, save and language continuation', async ({ page }) => {
  ...
  const map = page.locator('.career-canvas'); await map.focus();
  ...
  await expect(map).toHaveAttribute('data-player', '4,4');
  ...
  await expect(page.locator('.career-canvas')).toHaveAttribute('data-player', '4,4');
  await page.getByRole('button', { name: 'Caminar al este', exact: true }).click();
  await expect(page.locator('.career-canvas')).toHaveAttribute('data-player', '5,4');
  ...
  await page.getByRole('button', { name: 'Volver a la misión' }).click();
  await expect(page.getByRole('button', { name: 'Siguiente capítulo' })).toBeVisible();
...
test('tapping an elevated terminal sprite walks to it and opens its mission', async ({ page }) => {
  ...
  const map = page.locator('.career-canvas'); const box = (await map.boundingBox())!;
  await map.click({ position: { x: 387 / 720 * box.width, y: 142 / 440 * box.height } });
  await expect(page.getByRole('dialog')).toContainText('Connect the interaction');
  await expect(map).not.toHaveAttribute('data-player', '4,7');
});
...
test('static Spanish and English launchers expose the real career and CV without JavaScript', async ({ browser }) => {
  ...
  for (const prefix of ['', '/en']) {
    ...
    await expect(page.getByRole('heading', { name: 'AntoñiOS — Career Mode' })).toBeVisible();
