...
export function routes(content: ContentData) {
  ...
  return [...new Set([...applications.map(app => app.path), ...profileSections.map(section => section.path), ...projects.map(project => `/projects/${project.slug}/`), ...cases.map(item => `/architecture/${item.slug}/`), ...content.notes.map(note => `/notes/${note.slug}/`)])];
}
export function pageMetadata(path: string, content: ContentData) {
  ...
  const title = note?.title ?? project?.name ?? architectureCase?.title ?? (normalized === '/' || normalized === '/profile/' ? `${profile.name} · ${profile.role}` : section ? `${app.name} · ${t(section.name)}` : app.name);
  const description = note?.description ?? project?.description ?? architectureCase?.summary ?? (app.id === 'welcome' ? profile.statement : `${app.name} — ${app.description}. ${profile.shortName}, ${t('Software Architect')}.`);
  ...
  if (project || note || architectureCase) crumbs.push({ '@type': 'ListItem', position: 3, name: title, item: canonical });
