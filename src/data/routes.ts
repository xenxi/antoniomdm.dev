...
export function pageMetadata(path: string, content: ContentData) {
  ...
  const routeApp = normalized === '/arcade/' ? { name: 'AntoñiOS — Career Mode', description: locale === 'es' ? 'Una aventura RPG pixel art a color. Explora una pequeña ciudad con parques y desbloquea empresas siguiendo una carrera real. CV accesible sin jugar' : 'A colorful pixel art RPG adventure. Explore a small town with parks and unlock companies through a real career. View the CV without playing' } : normalized === '/architecture/' ? { name: 'Architecture', description: 'Engineering decisions from discovery to production' } : normalized === '/contact/' ? { name: 'Contact', description: 'Verified professional channels' } : undefined;
