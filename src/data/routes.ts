...
export function pageMetadata(path: string, content: ContentData) {
  ...
  const routeApp = normalized === '/arcade/' ? { name: 'AntoñiOS — Career Mode', description: locale === 'es' ? 'Una aventura RPG pixel art a color. Explora una pequeña ciudad con parques y desbloquea empresas siguiendo una carrera real. Perfil accesible sin jugar' : 'A colorful pixel art RPG adventure. Explore a small town with parks and unlock companies through a real career. View the profile without playing' } : normalized === '/architecture/' ? { name: 'Architecture', description: 'Engineering decisions from discovery to production' } : normalized === '/contact/' ? { name: 'Contact', description: 'Verified professional channels' } : undefined;
  ...
  const aiLabCase = getAiLabCases(locale).find(item => item.slug && normalized === `/ai-lab/${item.slug}/`);
  const isAiLabLanding = normalized === '/ai-lab/';
  ...
  const title = note?.title ?? project?.name ?? architectureCase?.title ?? aiLabCase?.title ?? (isAiLabLanding ? `${aiLabLandingContent.title[locale]} · AI Lab` : normalized === '/' || normalized === '/profile/' ? `${profile.name} · ${profile.role}` : section ? `${app.name} · ${t(section.name)}` : app.name);
  const description = note?.description ?? project?.description ?? architectureCase?.summary ?? aiLabCase?.summary ?? (isAiLabLanding ? aiLabLandingContent.metaDescription[locale] : app.id === 'welcome' ? profile.statement : `${app.name} — ${app.description}. ${profile.shortName}, ${t('Software Architect')}.`);
  ...
  else if (architectureCase) schema.push({ '@type': 'Article', headline: title, description, inLanguage: locale, author: { '@id': person['@id'] }, mainEntityOfPage: canonical, about: architectureCase.competencyIds });
  else if (aiLabCase) schema.push({ '@type': 'Article', headline: title, description, inLanguage: locale, author: { '@id': person['@id'] }, mainEntityOfPage: canonical, about: [aiLabCase.type, aiLabCase.depth] });
  else if (project) schema.push({ '@type': 'SoftwareSourceCode', name: project.name, description, programmingLanguage: project.technologies.filter(item => ['C#', 'Kotlin', 'Dart', 'TypeScript', 'JavaScript', 'Python'].includes(item)), url: canonical, author: { '@id': person['@id'] } });
  ...
  if (project || note || architectureCase || aiLabCase) crumbs.push({ '@type': 'ListItem', position: 3, name: title, item: canonical });
