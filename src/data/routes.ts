...
export function routes(content: ContentData) {
  ...
  return [...applications.map(app => app.path), ...projects.map(project => `/projects/${project.slug}/`), ...content.notes.map(note => `/notes/${note.slug}/`)];
}
export function pageMetadata(path: string, content: ContentData) {
  ...
  const app = { ...originalApp, name: t(originalApp.name), description: t(originalApp.description) };
  ...
  const schema: Record<string, unknown>[] = [person, { '@type': 'WebSite', '@id': 'https://antoniomdm.dev/#website', name: 'ANTONIOMDM OS', url: 'https://antoniomdm.dev/' }];
  if (note) schema.push({ '@type': 'Article', headline: title, description, inLanguage: locale, datePublished: note.date, dateModified: note.updated ?? note.date, author: { '@id': person['@id'] }, mainEntityOfPage: canonical, keywords: note.tags, image: `https://antoniomdm.dev${localizedPath('/social.png', locale)}` });
  ...
  if (normalized !== '/') crumbs.push({ '@type': 'ListItem', position: 2, name: app.name, item: `https://antoniomdm.dev${localizedPath(app.path, locale)}` });
