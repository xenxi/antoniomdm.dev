...
export const defaults: Preferences = { wallpaper: 'nebula', effects: true, sound: true, music: false, uiSounds: false };
...
export function parsePreferences(value: string | null): Preferences {
  try {
    ...
    return { wallpaper: data.wallpaper === 'midnight' ? 'midnight' : 'nebula',
      ...
      sound: typeof data.sound === 'boolean' ? data.sound : true, music: data.music === true, uiSounds: data.uiSounds === true };
