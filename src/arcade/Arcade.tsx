...
export default function Arcade({ content, portfolio, preferences, exit, navigate }: ArcadeProps) {
  ...
  return <section ref={ref} tabIndex={-1} class="arcade-world" aria-label={t("Arcade preview")} onKeyDown={event => { if (event.key === 'Escape') exit(); }}>
    ...
    <nav class="world-destinations" aria-label={t("World destinations")}><button onClick={() => navigate('/projects/')}>{t("01 / Projects")}{' '}<small>{projects[0].name}</small></button><button onClick={() => navigate('/experience/')}>{t("02 / Career")}{' '}<small>{experience[0].company}</small></button><button onClick={() => navigate('/notes/')}>{t("03 / Library")}{' '}<small>{content.notes.length} {t("note to discover")}</small></button><button onClick={() => navigate('/ai/')}>{t("04 / Lab")}{' '}<small>{t("Ideas in progress")}</small></button></nav>
