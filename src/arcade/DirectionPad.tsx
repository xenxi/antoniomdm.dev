...
export default function DirectionPad({ locale, active, onStep }: { locale: Locale; active: boolean; onStep: (dx: number, dy: number) => void }) {
  ...
  const stop = () => { clearInterval(repeat.current); repeat.current = undefined; pointer.current = null; setPressed(''); };
  ...
  return <div class="career-dpad" role="group" aria-label={copy.move[locale]}>{([
    ...
    if (!active) return;
    stop(); pointer.current = event.pointerId; setPressed(key); event.currentTarget.setPointerCapture(event.pointerId); step.current(dx, dy);
    repeat.current = window.setInterval(() => step.current(dx, dy), 140);
  }} onPointerUp={event => { if (pointer.current === event.pointerId) stop(); }} onPointerCancel={event => { if (pointer.current === event.pointerId) stop(); }} onLostPointerCapture={event => { if (pointer.current === event.pointerId) stop(); }} onClick={event => { if (event.detail === 0 && active) step.current(dx, dy); }}>{glyph}</button>)}</div>;
}
