...
export const hudCopy = {
  ...
  floor: text('PLANO DEL ESCENARIO', 'SCENE MAP'),
  floorRoute: text('Plano del escenario y ruta al objetivo actual', 'Scene floor plan and route to the current objective'),
...
export default function GameHud({ locale, map, signs, x, y, inside, currentId, mission, objective, energy, seconds, timed, completed, missionProgress = 0, missionTotal = 1, onAction }: { locale: Locale; map: PixelMap; signs: CompanySign[]; x: number; y: number; inside: boolean; currentId: string; mission?: string; objective?: string; energy?: number; seconds?: number; timed?: boolean; completed: number; missionProgress?: number; missionTotal?: number; onAction?: (action: string) => void }) {
  ...
  const position = { x, y };
  const destination = inside ? map.objects.find(o => o.id === (objective ?? 'terminal'))! : building.door;
