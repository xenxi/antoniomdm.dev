...
import { beginEvent, choose, completeEvent, eligibleEvents, replayChapter, getProgress, hasEvent, isComplete, newGame, newQuickMission, chapterUnlocked, chapterCleared, enterCompany, parseSave, saveKey, updateProgress, type EventId, type GameState, type ObjectId } from './engine';
...
export default function CareerGame({ data, preferences, exit }: ArcadeProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setGame(value => {
      ...
      if (progress.remaining === 0) return value;
      return updateProgress(value, { remaining: Math.max(0, (progress.remaining ?? seconds) - 1) });
    }), 1000);
  ...
  function interact(id: ObjectId | string) {
    ...
    if (id === 'coffee') { setGame(value => updateProgress({ ...value, coffee: true }, { energy: 100 })); setFeedback(qc(chapter.id === 'nokia' ? 'infinite' : 'coffee')); setModal('info'); return; }
    setFeedback(id === 'team' ? c('npcLine') : scene.description[locale]); setModal('info');
  }
  function solveSide(index: number) {
    if (!side) return;
    if (side.correct !== index) { setFeedback(qc('sideRetry')); return; }
    setGame(value => updateProgress(value, { side: [...new Set([...(getProgress(value).side ?? []), side.id])], energy: Math.min(100, (getProgress(value).energy ?? 100) + 20) }));
    setFeedback(side.result[locale]); setModal('info');
  }
  ...
  function answer(id: string) {
    ...
    else setGame(value => updateProgress(value, { energy: Math.max(0, (getProgress(value).energy ?? 100) - 5) }));
  ...
  return <section ref={host} class={`arcade-world career-world job-route ${screen === 'game' || screen === 'paused' ? 'is-playing' : 'is-menu'}`} tabIndex={-1} aria-label="AntoñiOS Career Mode" onPointerDown={() => { unlockEffects(); playMusic(); }} onKeyDown={e => {
    ...
    {screen === 'game' && modal === 'mission' && mission && !quickDone && <Modal title={mission.title[locale]} close={closeModal}><p class="eyebrow">{c(`${mission.mechanic}Label` as keyof typeof copy)}</p><p>{mission.briefing[locale]}</p><div class="mission-timer"><span>{game.timed !== false ? remaining + 's' : qc('untimed')}</span><button onClick={() => setGame(value => ({ ...value, timed: value.timed === false }))}>{game.timed !== false ? qc('untimed') : qc('timer')}</button></div>{timedOut && <div class="career-pending"><p role="status">{qc('timeout')}</p><button onClick={() => setGame(value => updateProgress(value, { remaining: seconds }))}>{qc('retry')}</button></div>}{mission.challenge === 'snake' && <SnakeQuest value={p.snake} locale={locale} disabled={timedOut} onChange={snake => setGame(value => updateProgress(value, { snake }))} />}{mission.evidence && <pre><code>{mission.evidence}</code></pre>}{mission.metrics && <div class="career-metrics">{mission.metrics.map(metric => <div key={metric.label}><label>{metric.label} <strong>{metric.value}%</strong><meter min={0} max={100} value={metric.value} /></label></div>)}</div>}{mission.sequence && <><p>{c('step')} {p.sequence.length + 1} / {mission.sequence.length}</p><div class="career-flow" aria-label={c('progress')}>{p.sequence.map(id => <span key={id}>✓ {mission.choices.find(choice => choice.id === id)!.label[locale]}</span>)}</div></>}<div class={`career-choices mechanic-${mission.mechanic}`}>{mission.choices.map(choice => <button key={choice.id} data-choice={choice.id} disabled={timedOut || p.sequence.includes(choice.id) || (mission.challenge === 'snake' && p.snake?.score !== 4)} onClick={() => answer(choice.id)}>{choice.label[locale]}</button>)}</div><p class="career-feedback" role="status">{feedback}</p><button onClick={() => setModal(null)}>{c('close')}</button></Modal>}
    {screen === 'game' && modal === 'side' && side && <Modal title={side.title[locale]} close={() => setModal(null)}><p class="eyebrow">{qc('optional')} · +20 {qc('energy')}</p><p>{side.briefing[locale]}</p><div class="career-choices">{side.options.map((option, index) => <button key={index} data-side-choice={index} onClick={() => solveSide(index)}>{option[locale]}</button>)}</div><p role="status">{feedback}</p><button onClick={() => setModal(null)}>{c('close')}</button></Modal>}
