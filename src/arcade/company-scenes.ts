...
export function makeCompanyMap(id: string, locale: Locale, company: string, resolved: string[] = [], mission = 0): PixelMap {
  ...
  for (const item of scene.furniture) {
    const p = project(item.x + .5, item.y + .5);
    if (item.kind === 'desk') map.effects.screens.push([p.x - tile * .33, p.y - tile * 1.08]);
    if (item.kind === 'rack') map.effects.lights.push([p.x - tile * .22, p.y - tile * 1.25, p.x - tile * .22, p.y - tile * .8, '#8ee6d8']);
    if (item.kind.startsWith('phone')) map.effects.lights.push([p.x - tile * .5, p.y, p.x + tile * .5, p.y + tile * .18, scene.accent]);
  }
  map.ambience = { notes: [], puddles: [] };
  if (id === 'freelance') {
    if (!resolved.includes('noise')) { const p = project(8.5, 7.5); map.ambience.notes.push([p.x, p.y - tile * 1.5]); }
    if (!resolved.includes('leak')) { const p = project(2, 2); map.ambience.puddles.push([p.x, p.y]); }
  }
  return map;
}
...
export function paintCompanyScene(map: PixelMap) {
  ...
  const entities = scene.furniture.map(item => {
    ...
    const shadow = npc ? ctx : g;
    shadow.fillStyle = '#07132355'; shadow.beginPath(); shadow.ellipse(px, py + 1, s * (npc ? .38 : .8), s * (npc ? .14 : .3), 0, 0, Math.PI * 2); shadow.fill();
    ...
    const dancing = kind === 'dancer' && !map.scene?.resolved.includes('foam');
    const motion = npc ? [0, 0, dancing ? 3.5 : kind === 'child' ? 2.2 : 1.5, x * 1.3 + y * .7, px, py, dancing ? .04 : .009, dancing ? .014 : .006] : [];
