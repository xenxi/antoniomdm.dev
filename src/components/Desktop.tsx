...
function DesktopContent({ path, content, data }: { path: string; content: ContentData; data: UiData }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (initialLayout.current && size.width >= 1100 && initialApp === 'about') dispatch({ type: 'geometry', id: 'about', viewport: size, rect: { x: 264, y: 18, width: Math.min(960, size.width - 284), height: Math.min(660, size.height - 30) } });
