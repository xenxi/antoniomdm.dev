...
import { pageMetadata, routes } from '../data/routes';
...
export default function Desktop({ path, content }: { path: string; content: ContentData }) {
  return <LocaleContext.Provider value={localeForPath(path)}><DesktopContent path={basePath(path)} content={content} /></LocaleContext.Provider>;
}
function DesktopContent({ path, content }: { path: string; content: ContentData }) {
  ...
  const knownPath = (value: string) => routes(content).includes(value);
  function syncMetadata(value: string) {
    const metadata = pageMetadata(href(value), content);
    document.title = `${metadata.title} · ANTONIOMDM OS`;
  ...
  return <div class={`os wallpaper-${preferences.wallpaper} ${preferences.effects ? '' : 'effects-off'}`} data-ready={ready} onClick={links} onKeyDown={keys}>
    ...
    <header class="system-bar"><a class="brand" href={href("/")}><span class="brand-symbol">a<span>m</span></span><strong>ANTONIOMDM <span>OS</span></strong><small>v1.0</small></a><span class="system-tagline">{t("Software Architect Playground")}</span><div class="system-right"><nav class="language-switch" aria-label={locale === 'es' ? 'Idioma' : 'Language'}>{(['es', 'en'] as const).map(language => <a key={language} data-language={language} href={localizedPath(activePath, language)} lang={language} hrefLang={language} aria-current={locale === language ? 'page' : undefined} aria-label={language === 'es' ? 'Español' : 'English'} onClick={event => { event.currentTarget.href = localizedPath(activePath, language) + location.search + location.hash; }}>{language.toUpperCase()}</a>)}</nav><span class="system-ready"><span class="status-dot" /> {t("SYSTEM READY")}</span><time>{clock || 'ANTONIOMDM OS'}</time></div></header>
    ...
    {mode === 'arcade' && Arcade && <Arcade content={content} preferences={preferences} exit={exitArcade} navigate={value => { exitArcade(); open(value); }} />}
  </div>;
}
