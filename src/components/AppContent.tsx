...
function ProjectDetail({ project }: { project: (ReturnType<typeof import('../data/portfolio').getPortfolio>)['projects'][number] }) {
  ...
  return <article class={`project-detail project-depth-${project.depth}`}><a class="back-link" href={href('/projects/')}>{t('← All projects')}</a><p class="eyebrow">{t('PERSONAL ENGINEERING LAB')} / {project.categoryLabel} / {project.depth}</p><div class="project-mark">{project.order.toString().padStart(2, '0')}</div><h1>{project.name}</h1><p class="lead">{project.description}</p><p class="project-status"><strong>{project.status}</strong></p><div class="tags">{project.technologies.map(technology => <span key={technology}>{technology}</span>)}</div>{isPlatform && <p class="project-note">{t('This project is personal evidence of experimentation, not professional employment evidence.')}</p>}<h2>{t('Why / intent')}</h2><p>{project.purpose}</p><h2>{t('Architectural decision')}</h2><p>{project.overview}</p><h2>{t('Implemented capabilities')}</h2><ul>{project.capabilities.map((item: string, index: number) => <li key={index}>{item}</li>)}</ul>{project.plannedCapabilities.length > 0 && <><h2>{t('Designed / planned')}</h2><ul class="project-planned">{project.plannedCapabilities.map((item: string, index: number) => <li key={index}><strong>{t('DESIGNED / PLANNED')}</strong> {item}</li>)}</ul></>}<h2>{t('Trade-offs / boundaries')}</h2><p>{project.tradeoff}</p><ul>{project.nonClaims.map((item: string, index: number) => <li key={index}>{item}</li>)}</ul><h2>{t('What it demonstrates')}</h2><p>{project.demonstration}</p><h2>{t('Links')}</h2><div class="actions">{project.links.map(link => <a class={link.kind === 'live' ? 'button primary' : 'button'} href={link.kind === 'detail' ? href(link.url) : link.url} target={link.kind === 'live' ? '_blank' : undefined} rel={link.kind === 'live' ? 'noreferrer' : undefined} key={link.url}>{link.label} ↗</a>)}</div></article>;
}
